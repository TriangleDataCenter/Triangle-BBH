...
def fisco_Mc_q(Mc, q):
    """ Mc in MSUN """
    return 4398.704053633259 * q ** (3./5.) / Mc  / (1. + q) ** (6./5.) 
...
class WaveformGeneratorFRef(WaveformGenerator):
    ...
    def __call__(self, parameters, Nfreqs=1024, fmin=1e-5, fmax=1e-1, fref=1e-3, freqs=None):
        ...
        self.fref = np.ones(Nevents) * fref
        ...
        phase_22_cut = phas_out[(2,2)][:, -1] # phase at fref (Nevents)
        for k, v in phas_out.items(): 
            # the mode-dependent part of phase correction 
            phase_mode_correction2 = 0.5 * k[1] * (-phase_22_cut + self.fref * (dphase_22_cut - TWOPI * parameters_in['reference_time'] * DAY)) # (Nevents) 
